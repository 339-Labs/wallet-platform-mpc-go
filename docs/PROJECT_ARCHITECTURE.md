...
│     │                         │                       │                   │      │
│     │                    ┌────┴────────────────────────────────────────────┐     │
│     │                    │     Round 2: MtA (乘法转加法) 协议               │     │
│     │                    │  - 执行 MtA: kᵢ * γⱼ → αᵢⱼ + βⱼᵢ               │     │
│     │                    │  - 执行 MtAwc: kᵢ * xⱼ → μᵢⱼ + νⱼᵢ            │     │
│     │                    │  - 点对点交换加密数据                            │     │
│     │                    └─────────────────────────────────────────────────┘     │
│     │                         │  p2p: MtA data       │                   │      │
│     │                         │ ◀────────────────────▶│                   │      │
...
#### MtA (Multiplicative-to-Additive) 协议详解

MtA 是 GG20 签名协议的核心，它允许两个参与方在不暴露各自秘密的情况下，将乘法关系转换为加法关系。
...
│  └─────────┬─────────┘                          │                                │
│            │                                    │                                │
│    结果: αᵢⱼ = a·b - β               结果: βⱼᵢ = β                              │
│    验证: αᵢⱼ + βⱼᵢ = (a·b - β) + β = a·b  ✓                                     │
│                                                                                  │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
| 设计点 | 决策 | 原因 |
|--------|------|------|
| TSS 库 | BNB Chain tss-lib | 成熟的 GG20 实现，安全性经过验证 |
